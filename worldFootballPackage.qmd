...
# Add the total injury count for each club
injuries_by_club_season <- injuries_by_club_season %>%
  group_by(club) %>%
  mutate(total_injury_count = sum(injury_count)) %>%
  ungroup()

# Create the interactive bar chart using Plotly with dynamic legend behavior
fig <- plot_ly(injuries_by_club_season, 
               x = ~season_injured,  # Move season_injured to the x-axis
               y = ~injury_count, 
               color = ~club,        # Color by club
               type = 'bar', 
               hoverinfo = 'x+y+text',
...
# Make the other clubs disappear when one is selected
fig <- fig %>%
  layout(
    legend = list(
      itemclick = "toggleothers"  # This will toggle other clubs when one is selected
    )
  )
